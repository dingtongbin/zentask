...
class UnstableTask(ZenBaseTask):
    ...
    async def on_retry(self, exception, retry_ctx, next_delay):
        ...
        print(f"  -> [Task {task_id}] 第 {self.attempts} 次失败: {type(exception).__name__}: {exception}")
    ...
    async def on_complete(self):
        task_id = self.kwargs.get('id')
        status_icon = "[SUCCESS]" if self.status.name == "SUCCESS" else "[FAILED]"
...
async def main():
    ...
    class PingTask(ZenBaseTask):
        ...
        def action(self, **kwargs):
            ...
            print(f"  [Worker] {job_id} 开始执行，预计延迟 {delay:.2f}s")
            time.sleep(delay)
